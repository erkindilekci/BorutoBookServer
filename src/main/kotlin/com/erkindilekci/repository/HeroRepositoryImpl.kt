package com.erkindilekci.repository

import com.erkindilekci.models.ApiResponse
import com.erkindilekci.models.Hero
import com.erkindilekci.util.Constants.NEXT_PAGE_KEY
import com.erkindilekci.util.Constants.PREV_PAGE_KEY
...
class HeroRepositoryImpl : HeroRepository {
    ...
    override suspend fun searchHeroes(searchQuery: String?): ApiResponse {
        return ApiResponse(
            success = true,
            message = "OK",
            heroes = findHeroes(searchQuery)
        )
    }

    private fun findHeroes(searchQuery: String?): List<Hero> {
        ...
        return if (searchQuery.isNullOrEmpty()) {
            ...
            heroes.forEach { _, heroes ->
                heroes.forEach { hero ->
                    if (hero.name.contains(searchQuery, ignoreCase = true)) {
